...
Pour comprendre la rupture technologique que représente Nanite, il faut d'abord identifier le goulot d'embouteillage (*bottleneck*) du pipeline graphique traditionnel. Classiquement, le processeur (CPU) doit envoyer des ordres de dessin (*Draw Calls*) à la carte graphique (GPU) pour chaque objet ou variation de détail (LOD). Si une scène contient trop d'objets ou des géométries extrêmement denses, le CPU sature. De plus, les architectures GPU actuelles gèrent très mal les triangles dont la taille devient inférieure à celle d'un pixel à l'écran, ce qui engendre un gaspillage massif de ressources de calcul (*sub-pixel shading*).
...
* **Un gouffre à calcul :** Nanite n'est absolument pas gratuit. Faire tourner des *Compute Shaders* en continu pour trier et rastériser des millions de clusters à la volée est infiniment plus lourd pour le GPU qu'un système de LOD classique (où la carte graphique se contente de dessiner des meshes déjà simplifiés).
* **L'overdraw généralisé :** L'overdraw, c'est quand le GPU doit calculer plusieurs fois le même pixel à l'écran parce que des couches de géométrie se superposent. Avec des clusters de triangles microscopiques imbriqués, l'overdraw explose et le GPU sature complètement à essayer de deviner ce qui est visible ou non et surtout à écrire "sans arrêt" le même pixel.
...
* **L'explosion du stockage et de la VRAM :** Même compressés, des modèles à plusieurs millions de polygones pèsent lourd sur le disque dur (les jeux dépassent vite les 100 Go) et demandent un streaming ultra-rapide depuis un SSD NVMe, sous peine de saturer instantanément la VRAM de ta carte graphique.
...
* **Des plateformes restreintes :** Nanite est gourmand, que ce soit intrinsèquement ou via ses dépendances. Utiliser Nanite sur son projet UE5, c'est limiter les plateformes supportées. Seuls le PC et les PlayStation 5 / Xbox Series S|X sont supportés de manière stable. Les consoles de la génération précédente (PS4) ont un support expérimental peu convaincant. Pour toutes les autres plateformes (mobiles, Android, casques VR, etc.), l'export est impossible, elles ne peuvent pas en profiter.
...
* **La surmultiplication des micro-triangles :** Comme Nanite découpe tout en clusters microscopiques pour coller à la résolution de l'écran, tu te retrouves avec une densité géométrique hallucinante. Si ces micro-triangles s'imbriquent et se superposent, le GPU passe un temps infini à recalculer et réécrire sans arrêt "par-dessus" les mêmes pixels pour essayer de deviner ce qui est finalement visible ou masqué.
* **Le cauchemar des masques d'opacité (Feuillage et Herbe) :** C'est le pire cas de figure. Quand tu utilises des masques d'opacité (des textures avec de la transparence pour découper la forme d'une feuille ou d'un brin d'herbe), Nanite perde son efficacité légendaire. Le moteur doit tester la transparence pixel par pixel à travers des dizaines de couches de micro-géométrie.
* **Le verdict du *Quad Overdraw* :** Cette surcharge met instantanément à genoux le pipeline. Dans les outils de debug de l'UE5, la vue *Quad Overdraw* (qui colore la scène du bleu au rouge/blanc selon la gravité du problème) vire direct au rouge écarlate. Le coût de calcul explose, le framerate s'effondre, et cela ruine complètement l'intérêt initial du système qui était censé fluidifier le rendu.
...
Le vrai problème se situe au niveau du *sub-pixel* et des quads de pixels. Une carte graphique ne calcule jamais les pixels un par un. Elle les traite par blocs de $2\times2$ pixels (un **Quad**). Pourquoi ? Pour pouvoir calculer les variations de textures (les dérivés) entre les pixels voisins.

C'est de là que vient le **drame du micro-triangle** : avec Nanite, si c'est loin et géométriquement dense, un triangle va être tellement minuscule qu'il ne va faire qu'**un seul pixel** à l'écran. Pour dessiner ce pauvre triangle d'un seul pixel, le GPU est obligé d'activer et de calculer le quad entier de **4 pixels** contenant ce fameux pixel. Les 3 autres pixels du bloc de calcul sont purement et simplement jetés à la poubelle (*Helper Pixels*). Et *bis repetita* pour le pixel à côté. Pour calculer le quad complet (si on a des triangles de 1 pixel), on aura calculé 16 pixels en tout, soit 4 fois plus de pixels.

En plus, si on a par exemple 5 couches de feuilles transparentes ou de micro-brins d'herbe les uns derrière les autres sur une zone dense, le Z-Buffer classique ne peut pas exclure les couches de derrière, car les textures ont des zones transparentes (masques d'opacité). Le GPU est obligé de lire la texture pour savoir si c'est transparent ou non. Chaque couche va forcer le GPU à lancer des calculs sur des Quads entiers ($2\times2$) pour de simples micro-triangles sur des zones de jeu entières. Le GPU passe alors son temps à mettre à la poubelle 3 pixels calculés sur 4 et à devoir calculer 4 fois le même quad pour avoir les 4 pixels le composant correctement affichés.
